...
class LConnectionMetadataInfo(object):
    ...
    def credential_types(self, credential_types):
        ...
        if not set(credential_types).issubset(set(allowed_values)):
            raise ValueError(
                "Invalid values for `credential_types` [{0}], must be a subset of [{1}]"  # noqa: E501
                .format(", ".join(map(str, set(credential_types) - set(allowed_values))),  # noqa: E501
