...
class LPublishInfo(object):
    ...
    """
    swagger_types = {
        'qualifying_path': 'list[str]',
        'supported_actions': 'list[str]',
        'supported_protocols': 'list[str]',
        'external_file_formats': 'list[str]'
    }

    attribute_map = {
        'qualifying_path': 'qualifyingPath',
        'supported_actions': 'supportedActions',
        'supported_protocols': 'supportedProtocols',
        'external_file_formats': 'externalFileFormats'
    }
    ...
    def __init__(self, qualifying_path=None, supported_actions=None, supported_protocols=None, external_file_formats=None):  # noqa: E501
        ...
        self._external_file_formats = None
        self.discriminator = None
        if qualifying_path is not None:
            self.qualifying_path = qualifying_path
        if supported_actions is not None:
            self.supported_actions = supported_actions
        if supported_protocols is not None:
            self.supported_protocols = supported_protocols
        if external_file_formats is not None:
            self.external_file_formats = external_file_formats
    ...
    def qualifying_path(self, qualifying_path):
        ...
        self._qualifying_path = qualifying_path
    ...
    def supported_actions(self, supported_actions):
        ...
        if not set(supported_actions).issubset(set(allowed_values)):
            raise ValueError(
                "Invalid values for `supported_actions` [{0}], must be a subset of [{1}]"  # noqa: E501
                .format(", ".join(map(str, set(supported_actions) - set(allowed_values))),  # noqa: E501
    ...
    def external_file_formats(self, external_file_formats):
        ...
        if not set(external_file_formats).issubset(set(allowed_values)):
            raise ValueError(
                "Invalid values for `external_file_formats` [{0}], must be a subset of [{1}]"  # noqa: E501
                .format(", ".join(map(str, set(external_file_formats) - set(allowed_values))),  # noqa: E501
                        ", ".join(map(str, allowed_values)))
            )

        self._external_file_formats = external_file_formats
