...
class LFlowNotificationSettingsCreateRequest(object):
    ...
    def on_job_failure(self, on_job_failure):
        ...
        if on_job_failure is None:
            raise ValueError("Invalid value for `on_job_failure`, must not be `None`")  # noqa: E501
        allowed_values = ["all", "scheduled", "adhoc", "never", "default"]  # noqa: E501
        if on_job_failure not in allowed_values:
            raise ValueError(
                "Invalid value for `on_job_failure` ({0}), must be one of {1}"  # noqa: E501
                .format(on_job_failure, allowed_values)
            )
    ...
    def on_job_success(self, on_job_success):
        ...
        if on_job_success is None:
            raise ValueError("Invalid value for `on_job_success`, must not be `None`")  # noqa: E501
        allowed_values = ["all", "scheduled", "adhoc", "never", "default"]  # noqa: E501
        if on_job_success not in allowed_values:
            raise ValueError(
                "Invalid value for `on_job_success` ({0}), must be one of {1}"  # noqa: E501
                .format(on_job_success, allowed_values)
            )
